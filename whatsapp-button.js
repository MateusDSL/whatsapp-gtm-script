// WhatsApp Button Script - VERSÃO COM EXPIRAÇÃO DE 1 DIA (CORRIGIDA)
(function() {
    // --- CONFIGURAÇÕES ---
    ...
    const EXPIRATION_TIME_IN_HOURS = 24; // Tempo de expiração em horas
    ...
    function closeModal() {
        ...
        }, 300); // Animação de 300ms para fechar
    }
    ...
    function openModal() {
        ...
        if (widgetWrapper) widgetWrapper.classList.remove('show'); // Esconde o botão ao abrir o modal
        isModalOpen = true;
        createModal();
    ...
    async function handleSubmit(e) {
       ...
       try {
        ...
        }, 2000); // Mostra "Enviado!" por 2 segundos
    }
    ...
    function createWidget() {
        ...
        if (submissionDataString) {
            ...
            const expirationInMs = EXPIRATION_TIME_IN_HOURS * 60 * 60 * 1000;
    ...
    function configureWidgetForDirectRedirect() {
        ...
        if(container) {
            createWidget(); // Recria o widget no estado correto
            setTimeout(() => {
                const widgetWrapper = document.querySelector('.whatsapp-widget-wrapper');
                if (widgetWrapper) widgetWrapper.classList.add('show'); // Mostra o novo widget
            }, 100);
        }
    }
